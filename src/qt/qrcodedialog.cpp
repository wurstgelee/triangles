...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TRI unit input in lnReqAmount, we generate the URI with TRI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TrianglesUnits::format(TrianglesUnits::TRI, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
